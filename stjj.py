# gets all files under ROOT_INPUT_PATH with FILE_EXTENSION and tries to extract text from them into ROOT_OUTPUT_PATH
# with same filename as the input file but with INPUT_FILE_EXTENSION replaced by OUTPUT_FILE_EXTENSION
...
def acrobat_extract_text(f_path, f_path_out, f_basename, f_ext):
    ...
    assert (
        ret)  # FIXME: Documentation says "-1 if the file was opened successfully, 0 otherwise", but this is a bool in practise?

    pdDoc = avDoc.GetPDDoc()

    dst = os.path.join(f_path_out, ''.join((f_basename, f_ext)))

    # Adobe documentation says "For that reason, you must rely on the documentation to know what functionality is
    # available through the JSObject interface. For details, see the JavaScript for Acrobat API Reference"
    ...
    pdDoc.Close()
    avDoc.Close(True)  # We want this to close Acrobat, as otherwise Acrobat is going to refuse processing any further
    # files after a certain threshold of open files are reached (for example 50 PDFs)
    del pdDoc


if __name__ == "__main__":
    assert (5 == len(sys.argv)), sys.argv  # <script name>, <script_file_input_path>, <script_file_input_extension>,
    # <script_file_output_path>, <script_file_output_extension>
    ...
    matching_files = ((os.path.join(_root, filename), os.path.splitext(filename)[0]) for _root, _dirs, _files in
                      walk(ROOT_INPUT_PATH) for filename in fnmatch.filter(_files, INPUT_FILE_EXTENSION))
